...
int Database::getEmbeddings(ArcFaceR100 &recognizer) {
    std::string sql;
    sqlite3_stmt *stmt;
    int rc;
    std::string userId;
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        // std::cout << embedding[0] << " " << embedding[10] << " " << std::endl;
...
int Database::exportToJson() {
    ...
    std::vector< std::pair <std::string, std::vector<float>>> vect;
    // init num embeds
    int numEmbeds = getNumEmbeddings();
    std::cout << "[INFO] There are " << numEmbeds << " embeddings in database\n";

    // get embedding
    sql = "SELECT a.USR_ID, b.EMBEDDING  FROM USER a, FACE b WHERE a.USR_ID=b.USR_ID;";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        userId = std::string(reinterpret_cast<const char *>(sqlite3_column_text(stmt, 0)));
        embedding = (float*)sqlite3_column_blob(stmt, 1);
        float *output = new float[512];
        memcpy(output, embedding, 512*sizeof(float));
        std::vector<float> vf{output, output + 512};
        vect.push_back(std::make_pair(userId, vf));
    }
    if (rc != SQLITE_DONE) {
        std::cout << "SQL error: " << sqlite3_errmsg(m_db);
    }
    ...
    // json to file
    std::map<std::string, std::vector<std::vector<float>>> c_map;
    for (const auto& pair : vect)
    {
        if (!c_map.count(pair.first)) {
            c_map[pair.first] = std::vector<std::vector<float>>();
        }
        c_map[pair.first].push_back(pair.second);
    }

    json j_map;
    for (auto i : c_map) {
        j_map[i.first] = i.second;
    }

    std::ofstream file("embedding.json");
    file << j_map;
